...
def home():
    ...
    if request.method == 'POST':
        ...
        if action == 'embed':
            ...
            secret_data.encoded_message = ''.join(
    str(
        int(secret[i]) ^
        int(secret_key[i])
    )
    for i in range(4)
        ...
        # =================================================
            ...
            if (
                ...
                try:
                    counts = ast.literal_eval(measurement_text)
                    recovered_message = extract_secret_message(
                        counts,
                        receiver_key
                    )

                    if receiver_key == secret_data.secret_key:
                        access_status = "Authorized Receiver"
                    else:
                        access_status = (
                            "Invalid Key - Message Recovery Failed"
                        )
                except:
                    recovered_message = (
